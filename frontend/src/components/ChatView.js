...
const ChatView = () => {
  ...
  const sendMessage = async (e) => {
    ...
    const url = "http://localhost:5000?search=" + cleanPrompt;
    const response = await axios.get(url).catch(r => {
  ...
  const updatePrompt = async () => {
    const api = 'http://us-central1-prompt-ops.cloudfunctions.net/optimize';
